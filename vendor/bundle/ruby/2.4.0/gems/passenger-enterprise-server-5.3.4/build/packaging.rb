...
task 'package:list_enterprise' do
  ...
  sh "ssh passenger_website@www.phusionpassenger.com ls \"#{dir}/assets\""
end
...
task 'package:update_homebrew' do
  ...
  sha256 = `brew fetch "#{homebrew_tap_dir}/Formula/passenger-enterprise.rb"`.each_line.select{|e|e.include? "SHA"}.first.split[1]
...
task 'package:update_homebrew_tap' do
  version = VERSION_STRING
  if !File.exist?(homebrew_tap_dir)
    sh "git clone git@github.com:phusion/homebrew-passenger.git #{homebrew_tap_dir}"
  end
  sh "cd #{homebrew_tap_dir} && git fetch"
  sh "cd #{homebrew_tap_dir} && git reset --hard origin/master"
  formula = File.read("#{homebrew_tap_dir}/Formula/passenger-enterprise.rb")
  formula.gsub!(/^  version "[0-9\.]*"/, "  version \"#{version}\"") ||
    abort("Unable to substitute Homebrew version number")
  File.open("#{homebrew_tap_dir}/Formula/passenger-enterprise.rb", "w") do |f|
    f.write(formula)
  end

  sha256 = `brew fetch "#{homebrew_tap_dir}/Formula/passenger-enterprise.rb"`.each_line.select{|e|e.include? "SHA"}.first.split[1]

  formula.gsub!(/^  sha256 "[0-9a-fA-F]*"/, "  sha256 \"#{sha256}\"") ||
    abort("Unable to substitute Homebrew formula SHA-256")
  necessary_dirs = ORIG_TARBALL_FILES.call.map{ |filename| filename.split("/").first }.uniq
  necessary_dirs -= Packaging::HOMEBREW_EXCLUDE
  necessary_dirs += ["buildout"]
  ...
  sh "cd #{homebrew_tap_dir} && git commit -a -m 'passenger #{version}'"
  sh "cd #{homebrew_tap_dir} && git push -f"
  if boolean_option('HOMEBREW_TEST', true)
    sh "cp #{homebrew_tap_dir}/Formula/passenger-enterprise.rb $(brew --prefix)/Homebrew/Library/Taps/phusion/homebrew-passenger/Formula/passenger-enterprise.rb"
    if `brew info nginx` !~ /^Not installed$/
      sh "brew uninstall nginx"
    end
    if `brew info nginx-passenger-enterprise` !~ /^Not installed$/
    ...
    sh "cp #{PKG_DIR}/passenger-enterprise-server-#{version}.tar.gz `brew --cache`/passenger-enterprise-#{version}.tar.gz"
    sh "brew install passenger-enterprise"
    sh "brew install nginx-passenger-enterprise"
    Rake::Task['test:integration:native_packaging'].invoke
  end
end
