    ...
    class StartCommand
      ...
      private
        def start_engine_real
          if @options[:defer_port_binding]
            @options[:ctls] << "defer_port_binding=#{@options[:defer_port_binding]}"
            @options[:ctls] << "defer_port_binding_params_file=#{@working_dir}/port_binding_proxy_params"
            @options[:ctls] << "defer_port_binding_timeout=#{@options[:start_timeout]}"
          end
          ...
          if @options[:defer_port_binding]
            proxy_controller_options = dump_port_binding_proxy_params
            @proxy_controller = DaemonController.new(proxy_controller_options)
          end
        ...
        def dump_port_binding_proxy_params
          old_options = @options.dup
          @options[:pid_file] = "#{@options[:pid_file]}.proxy"
          @options[:log_file] = "#{@options[:log_file]}.proxy"

          write_nginx_tcp_to_unix_proxy_file(nginx_tcp_to_unix_proxy_path)
          maybe_debug_nginx_config(nginx_tcp_to_unix_proxy_path)
          test_nginx_config(nginx_tcp_to_unix_proxy_path, 'nginx_tcp_to_unix_proxy.conf')

          proxy_controller_options = build_proxy_daemon_controller_options
          File.open("#{@working_dir}/port_binding_proxy_params", 'wb') do |f|
            f.write(Marshal.dump(proxy_controller_options))
          end

          @options = old_options
          proxy_controller_options
        end
        ...
        def wait_until_engine_has_exited
          ...
          while true
            if @options[:socket_file] || @options[:defer_port_binding]
        ...
        def build_proxy_daemon_controller_options
          return {
            :identifier    => 'Port binding proxy',
            :start_command => "#{Shellwords.escape @nginx_binary} " +
              "-c #{Shellwords.escape nginx_tcp_to_unix_proxy_path} " +
              "-p #{Shellwords.escape @working_dir}",
            :stop_command => "#{Shellwords.escape @nginx_binary} " +
              "-c #{Shellwords.escape nginx_tcp_to_unix_proxy_path} " +
              "-p #{Shellwords.escape @working_dir} " +
              "-s quit",
            :ping_command  => [:tcp, @options[:address], @options[:port]],
            :pid_file      => @options[:pid_file],
            :log_file      => @options[:log_file],
            :start_timeout => 25,
            :stop_timeout  => 60,
        ...
        def nginx_tcp_to_unix_proxy_path
          return "#{@working_dir}/nginx_tcp_to_unix_proxy.conf"
        end
        ...
        def write_nginx_tcp_to_unix_proxy_file(path)
          File.open(path, 'w') do |f|
            f.chmod(0644)
            erb = ERB.new(File.read(nginx_tcp_to_unix_proxy_template_filename), nil,
              "-", next_eoutvar)
            erb.filename = nginx_tcp_to_unix_proxy_template_filename
        ...
        def nginx_tcp_to_unix_proxy_template_filename
            return File.join(PhusionPassenger.resources_dir,
              "templates", "standalone", "tcp_to_unix_proxy.erb")
        end
