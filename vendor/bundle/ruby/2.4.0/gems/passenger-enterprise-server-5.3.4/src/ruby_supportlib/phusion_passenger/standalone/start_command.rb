    ...
    private
      ...
      def sanity_check_options_and_set_defaults
        ...
        if @options[:defer_port_binding]
          create_working_dir
          # Be careful with the socket name, the whole path can't be longer than 104 chars uncluding the 'unix:' protocol.
          # OS' with private tmp dirs will have long paths to the tmp dir such as: /var/folders/0x/xxxxxxxxxxxxxxxxxxxxxxxxxxxxxx/T/
          # and our instance dir name is rather long such as passenger-standalone.xxxxxxx
          @options[:socket_file] ||= Pathname.new(@working_dir).join("tu_proxy.#{@options[:port]}").to_s
        end
      end
      ...
      # for the given app.
      def listen_url(app)
        if app[:socket_file] || app[:defer_port_binding]
      ...
      def trapsafe_shutdown_and_cleanup(error_occurred)
        ...
        if @engine && (error_occurred || should_wait_until_engine_has_exited?)
          @console_mutex.synchronize do
            begin
              if @proxy_controller
                STDOUT.write("Stopping port binding proxy...")
                STDOUT.flush
