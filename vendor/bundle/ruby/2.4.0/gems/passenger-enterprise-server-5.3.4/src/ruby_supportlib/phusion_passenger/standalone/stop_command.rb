    ...
    private
      def self.create_option_parser(options)
        OptionParser.new do |opts|
          ...
          opts.on("--defer-port-binding", "-d",
            "Shutdown deferred port binding proxy") do |value|
            options[:defer_port_binding] = true
          end
      ...
      def create_proxy_controller
        Standalone::ControlUtils.require_daemon_controller
        @proxy_controller = DaemonController.new(
          :identifier    => 'Port binding proxy',
          :start_command => "true", # Doesn't matter
          :ping_command  => "true", # Doesn't matter
          :pid_file      => "#{@options[:pid_file]}.proxy",
          :log_file      => "/dev/null",
          :timeout       => 25
        )
      end
