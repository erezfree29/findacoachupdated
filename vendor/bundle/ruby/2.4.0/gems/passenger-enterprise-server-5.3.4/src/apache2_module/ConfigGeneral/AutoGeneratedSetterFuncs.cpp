...
static const char *
cmd_passenger_app_log_file(cmd_parms *cmd, void *pcfg, const char *arg) {
	const char *err = ap_check_cmd_context(cmd, NOT_IN_FILES);
	if (err != NULL) {
		return err;
	}
	...
	DirConfig *config = (DirConfig *) pcfg;
	config->mAppLogFileSourceFile = cmd->directive->filename;
	config->mAppLogFileSourceLine = cmd->directive->line_num;
	config->mAppLogFileExplicitlySet = true;
	config->mAppLogFile = arg;
	return NULL;
}
...
static const char *
cmd_passenger_concurrency_model(cmd_parms *cmd, void *pcfg, const char *arg) {
	const char *err = ap_check_cmd_context(cmd, NOT_IN_FILES);
	if (err != NULL) {
		return err;
	}
	...
	DirConfig *config = (DirConfig *) pcfg;
	config->mConcurrencyModelSourceFile = cmd->directive->filename;
	config->mConcurrencyModelSourceLine = cmd->directive->line_num;
	config->mConcurrencyModelExplicitlySet = true;
	config->mConcurrencyModel = arg;
	return NULL;
}
...
static const char *
cmd_passenger_debugger(cmd_parms *cmd, void *pcfg, const char *arg) {
	const char *err = ap_check_cmd_context(cmd, NOT_IN_FILES);
	if (err != NULL) {
		return err;
	}
	...
	DirConfig *config = (DirConfig *) pcfg;
	config->mDebuggerSourceFile = cmd->directive->filename;
	config->mDebuggerSourceLine = cmd->directive->line_num;
	config->mDebuggerExplicitlySet = true;
	config->mDebugger =
		(arg != NULL) ?
		ENABLED :
		DISABLED;
...
static const char *
cmd_passenger_fly_with(cmd_parms *cmd, void *pcfg, const char *arg) {
	const char *err = ap_check_cmd_context(cmd, GLOBAL_ONLY);
	if (err != NULL) {
		ap_log_perror(APLOG_MARK, APLOG_STARTUP, 0, cmd->temp_pool,
			"WARNING: %s", err);
	...
	serverConfig.flyWithSourceFile = cmd->directive->filename;
	serverConfig.flyWithSourceLine = cmd->directive->line_num;
	serverConfig.flyWithExplicitlySet = true;
	serverConfig.flyWith = arg;
	return NULL;
}
...
static const char *
cmd_passenger_max_instances(cmd_parms *cmd, void *pcfg, const char *arg) {
	const char *err = ap_check_cmd_context(cmd, NOT_IN_FILES);
	if (err != NULL) {
		return err;
	}
	...
	DirConfig *config = (DirConfig *) pcfg;
	config->mMaxInstancesSourceFile = cmd->directive->filename;
	config->mMaxInstancesSourceLine = cmd->directive->line_num;
	config->mMaxInstancesExplicitlySet = true;
	return setIntConfig(cmd, arg, config->mMaxInstances, 0);
}
...
cmd_passenger_max_request_queue_time(cmd_parms *cmd, void *pcfg, const char *arg) {
	DirConfig *config = (DirConfig *) pcfg;
	config->mMaxRequestQueueTimeSourceFile = cmd->directive->filename;
	config->mMaxRequestQueueTimeSourceLine = cmd->directive->line_num;
	config->mMaxRequestQueueTimeExplicitlySet = true;
	return setIntConfig(cmd, arg, config->mMaxRequestQueueTime, 0);
}

static const char *
cmd_passenger_max_request_time(cmd_parms *cmd, void *pcfg, const char *arg) {
	DirConfig *config = (DirConfig *) pcfg;
	config->mMaxRequestTimeSourceFile = cmd->directive->filename;
	config->mMaxRequestTimeSourceLine = cmd->directive->line_num;
	config->mMaxRequestTimeExplicitlySet = true;
	return setIntConfig(cmd, arg, config->mMaxRequestTime, 0);
}
...
cmd_passenger_memory_limit(cmd_parms *cmd, void *pcfg, const char *arg) {
	...
	DirConfig *config = (DirConfig *) pcfg;
	config->mMemoryLimitSourceFile = cmd->directive->filename;
	config->mMemoryLimitSourceLine = cmd->directive->line_num;
	config->mMemoryLimitExplicitlySet = true;
	return setIntConfig(cmd, arg, config->mMemoryLimit, 0);
}
...
static const char *
cmd_passenger_resist_deployment_errors(cmd_parms *cmd, void *pcfg, const char *arg) {
	const char *err = ap_check_cmd_context(cmd, NOT_IN_FILES);
	if (err != NULL) {
		return err;
	}
	...
	DirConfig *config = (DirConfig *) pcfg;
	config->mResistDeploymentErrorsSourceFile = cmd->directive->filename;
	config->mResistDeploymentErrorsSourceLine = cmd->directive->line_num;
	config->mResistDeploymentErrorsExplicitlySet = true;
...
static const char *
cmd_passenger_rolling_restarts(cmd_parms *cmd, void *pcfg, const char *arg) {
	const char *err = ap_check_cmd_context(cmd, NOT_IN_FILES);
	if (err != NULL) {
		return err;
	}
	...
	DirConfig *config = (DirConfig *) pcfg;
	config->mRollingRestartsSourceFile = cmd->directive->filename;
	config->mRollingRestartsSourceLine = cmd->directive->line_num;
	config->mRollingRestartsExplicitlySet = true;
...
static const char *
cmd_passenger_thread_count(cmd_parms *cmd, void *pcfg, const char *arg) {
	const char *err = ap_check_cmd_context(cmd, NOT_IN_FILES);
	if (err != NULL) {
		return err;
	}
	...
	DirConfig *config = (DirConfig *) pcfg;
	config->mThreadCountSourceFile = cmd->directive->filename;
	config->mThreadCountSourceLine = cmd->directive->line_num;
	config->mThreadCountExplicitlySet = true;
	return setIntConfig(cmd, arg, config->mThreadCount, 0);
}
