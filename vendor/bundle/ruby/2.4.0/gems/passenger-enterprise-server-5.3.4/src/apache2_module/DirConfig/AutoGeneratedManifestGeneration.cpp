...
ConfigManifestGenerator::autoGenerated_generateConfigManifestForDirConfig(server_rec *serverRec, core_server_config *csconf,core_dir_config *cdconf, DirConfig *pdconf, DirConfigContext context) {
	...
	if (pdconf->mConcurrencyModelExplicitlySet) {
		findOrCreateAppAndLocOptionsContainers(serverRec, csconf, cdconf,
			pdconf, context, &appOptionsContainer, &locOptionsContainer);
		Json::Value &optionContainer = findOrCreateOptionContainer(*appOptionsContainer,
			"PassengerConcurrencyModel",
			sizeof("PassengerConcurrencyModel") - 1);
		Json::Value &hierarchyMember = addOptionContainerHierarchyMember(optionContainer,
			pdconf->mConcurrencyModelSourceFile,
			pdconf->mConcurrencyModelSourceLine);
		hierarchyMember["value"] = Json::Value(
			pdconf->mConcurrencyModel.data(),
			pdconf->mConcurrencyModel.data() + pdconf->mConcurrencyModel.size());
	}
	if (pdconf->mDebuggerExplicitlySet) {
		findOrCreateAppAndLocOptionsContainers(serverRec, csconf, cdconf,
			pdconf, context, &appOptionsContainer, &locOptionsContainer);
		Json::Value &optionContainer = findOrCreateOptionContainer(*appOptionsContainer,
			"PassengerDebugger",
			sizeof("PassengerDebugger") - 1);
		Json::Value &hierarchyMember = addOptionContainerHierarchyMember(optionContainer,
			pdconf->mDebuggerSourceFile,
			pdconf->mDebuggerSourceLine);
		hierarchyMember["value"] = pdconf->mDebugger == Apache2Module::ENABLED;
	}
	...
	if (pdconf->mMaxInstancesExplicitlySet) {
		findOrCreateAppAndLocOptionsContainers(serverRec, csconf, cdconf,
			pdconf, context, &appOptionsContainer, &locOptionsContainer);
		Json::Value &optionContainer = findOrCreateOptionContainer(*appOptionsContainer,
			"PassengerMaxInstances",
			sizeof("PassengerMaxInstances") - 1);
		Json::Value &hierarchyMember = addOptionContainerHierarchyMember(optionContainer,
			pdconf->mMaxInstancesSourceFile,
			pdconf->mMaxInstancesSourceLine);
		hierarchyMember["value"] = pdconf->mMaxInstances;
	}
	...
	if (pdconf->mMaxRequestQueueTimeExplicitlySet) {
		findOrCreateAppAndLocOptionsContainers(serverRec, csconf, cdconf,
			pdconf, context, &appOptionsContainer, &locOptionsContainer);
		Json::Value &optionContainer = findOrCreateOptionContainer(*locOptionsContainer,
			"PassengerMaxRequestQueueTime",
			sizeof("PassengerMaxRequestQueueTime") - 1);
		Json::Value &hierarchyMember = addOptionContainerHierarchyMember(optionContainer,
			pdconf->mMaxRequestQueueTimeSourceFile,
			pdconf->mMaxRequestQueueTimeSourceLine);
		hierarchyMember["value"] = pdconf->mMaxRequestQueueTime;
	}
	if (pdconf->mMaxRequestTimeExplicitlySet) {
		findOrCreateAppAndLocOptionsContainers(serverRec, csconf, cdconf,
			pdconf, context, &appOptionsContainer, &locOptionsContainer);
		Json::Value &optionContainer = findOrCreateOptionContainer(*locOptionsContainer,
			"PassengerMaxRequestTime",
			sizeof("PassengerMaxRequestTime") - 1);
		Json::Value &hierarchyMember = addOptionContainerHierarchyMember(optionContainer,
			pdconf->mMaxRequestTimeSourceFile,
			pdconf->mMaxRequestTimeSourceLine);
		hierarchyMember["value"] = pdconf->mMaxRequestTime;
	}
	...
	if (pdconf->mMemoryLimitExplicitlySet) {
		findOrCreateAppAndLocOptionsContainers(serverRec, csconf, cdconf,
			pdconf, context, &appOptionsContainer, &locOptionsContainer);
		Json::Value &optionContainer = findOrCreateOptionContainer(*appOptionsContainer,
			"PassengerMemoryLimit",
			sizeof("PassengerMemoryLimit") - 1);
		Json::Value &hierarchyMember = addOptionContainerHierarchyMember(optionContainer,
			pdconf->mMemoryLimitSourceFile,
			pdconf->mMemoryLimitSourceLine);
		hierarchyMember["value"] = pdconf->mMemoryLimit;
	}
	...
	if (pdconf->mResistDeploymentErrorsExplicitlySet) {
		findOrCreateAppAndLocOptionsContainers(serverRec, csconf, cdconf,
			pdconf, context, &appOptionsContainer, &locOptionsContainer);
		Json::Value &optionContainer = findOrCreateOptionContainer(*appOptionsContainer,
			"PassengerResistDeploymentErrors",
			sizeof("PassengerResistDeploymentErrors") - 1);
		Json::Value &hierarchyMember = addOptionContainerHierarchyMember(optionContainer,
			pdconf->mResistDeploymentErrorsSourceFile,
			pdconf->mResistDeploymentErrorsSourceLine);
		hierarchyMember["value"] = pdconf->mResistDeploymentErrors == Apache2Module::ENABLED;
	}
	...
	if (pdconf->mRollingRestartsExplicitlySet) {
		findOrCreateAppAndLocOptionsContainers(serverRec, csconf, cdconf,
			pdconf, context, &appOptionsContainer, &locOptionsContainer);
		Json::Value &optionContainer = findOrCreateOptionContainer(*appOptionsContainer,
			"PassengerRollingRestarts",
			sizeof("PassengerRollingRestarts") - 1);
		Json::Value &hierarchyMember = addOptionContainerHierarchyMember(optionContainer,
			pdconf->mRollingRestartsSourceFile,
			pdconf->mRollingRestartsSourceLine);
		hierarchyMember["value"] = pdconf->mRollingRestarts == Apache2Module::ENABLED;
	}
	...
	if (pdconf->mThreadCountExplicitlySet) {
		findOrCreateAppAndLocOptionsContainers(serverRec, csconf, cdconf,
			pdconf, context, &appOptionsContainer, &locOptionsContainer);
		Json::Value &optionContainer = findOrCreateOptionContainer(*appOptionsContainer,
			"PassengerThreadCount",
			sizeof("PassengerThreadCount") - 1);
		Json::Value &hierarchyMember = addOptionContainerHierarchyMember(optionContainer,
			pdconf->mThreadCountSourceFile,
			pdconf->mThreadCountSourceLine);
		hierarchyMember["value"] = pdconf->mThreadCount;
	}
