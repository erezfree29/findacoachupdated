...
parseCoreOption(int argc, const char *argv[], int &i, Json::Value &updates) {
	...
	if (p.isValueFlag(argc, i, argv[i], '\0', "--passenger-root")) {
		...
		updates["default_max_request_queue_time"] = atoi(argv[i + 1]);
		i += 2;
		...
		i += 2;
	} else if (p.isFlag(argv[i], '\0', "--defer-port-binding")) {
		updates["defer_port_binding"] = true;
		i++;
	} else if (p.isValueFlag(argc, i, argv[i], '\0', "--start-timeout")) {
		updates["defer_port_binding_timeout"] = atoi(argv[i + 1]);
		i += 2;
