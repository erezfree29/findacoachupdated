 ...
 *   default_app_thread_count                            unsigned integer   -          default(1)
 *   default_concurrency_model                           string             -          default("process")
 *   default_debugger_support                            boolean            -          default(false)
 ...
 *   default_resist_deployment_errors                    boolean            -          default(false)
 *   default_rolling_restarts                            boolean            -          default(false)
...
private:
	void initialize() {
		...
		add("default_max_request_queue_time", UINT_TYPE, OPTIONAL, DEFAULT_MAX_REQUEST_QUEUE_TIME);
		...
		/*******************/
		add("enabled", BOOL_TYPE, OPTIONAL, true);
		add("max_request_time", UINT_TYPE, OPTIONAL, 0);
		add("default_max_instances", UINT_TYPE, OPTIONAL, 0);
		add("default_debugger_support", BOOL_TYPE, OPTIONAL, false);
		add("default_concurrency_model", STRING_TYPE, OPTIONAL, DEFAULT_CONCURRENCY_MODEL);
		add("default_app_thread_count", UINT_TYPE, OPTIONAL, DEFAULT_APP_THREAD_COUNT);
		add("default_memory_limit", UINT_TYPE, OPTIONAL, 0);
		add("default_rolling_restarts", BOOL_TYPE, OPTIONAL, false);
		add("default_resist_deployment_errors", BOOL_TYPE, OPTIONAL, false);
		add("hook_max_request_time_reached", STRING_TYPE, OPTIONAL);
		/*******************/
...
public:
	...
	unsigned int defaultMaxRequestQueueTimeMs;
	...
	/*******************/
	StaticString defaultConcurrencyModel;
	unsigned int maxRequestTime;
	unsigned int defaultAppThreadCount;
	unsigned int defaultMemoryLimit;
	unsigned int defaultMaxInstances;
	bool defaultDebuggerSupport: 1;
	bool defaultRollingRestarts: 1;
	bool defaultResistDeploymentErrors: 1;
	/*******************/
	...
	ControllerRequestConfig(const ConfigKit::Store &config)
		: pool(psg_create_pool(1024 * 4)),
		  ...
		  defaultMaxRequestQueueTimeMs(config["max_request_queue_time"].asUInt()),
		  ...
		  /*******************/
		  ,
		  defaultConcurrencyModel(psg_pstrdup(pool, config["default_concurrency_model"].asString())),
		  maxRequestTime(config["max_request_time"].asUInt()),
		  defaultAppThreadCount(config["default_app_thread_count"].asUInt()),
		  defaultMemoryLimit(config["default_memory_limit"].asUInt()),
		  defaultMaxInstances(config["default_max_instances"].asUInt()),
		  defaultDebuggerSupport(config["debugger_support"].asBool()),
		  defaultRollingRestarts(config["default_rolling_restarts"].asBool()),
		  defaultResistDeploymentErrors(config["default_resist_deployment_errors"].asBool())
		{ }
