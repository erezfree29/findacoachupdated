	...
	psg_pool_t *pool, const ControllerRequestConfigPtr &requestConfig)
{
	...
	options.maxRequestQueueTimeMs = requestConfig->defaultMaxRequestQueueTimeMs;
	...
	options.debugger = requestConfig->defaultDebuggerSupport;
	options.concurrencyModel = requestConfig->defaultConcurrencyModel;
	options.threadCount = requestConfig->defaultAppThreadCount;
	options.memoryLimit = requestConfig->defaultMemoryLimit;
	options.rollingRestart = requestConfig->defaultRollingRestarts;
	options.ignoreSpawnErrors = requestConfig->defaultResistDeploymentErrors;
	options.maxProcesses = requestConfig->defaultMaxInstances;
}
	...
	const HashedStaticString &appGroupName)
{
	...
	fillPoolOptionSecToMsec(req, options.maxRequestQueueTimeMs, "!~PASSENGER_MAX_REQUEST_QUEUE_TIME");
...
Controller::onRequestBegin(Client *client, Request *req) {
...

	{
		...
		if (OXT_UNLIKELY(!mainConfig.enabled)) {
			endRequestWithSimpleResponse(&client, &req,
				SHORT_PROGRAM_NAME " application server disabled; see logfile for further information.",
				500);
			return;
		}
...
Controller::disable(const string &reason) {
	...
	if (prepareConfigChange(updates, errors, req)) {
		commitConfigChange(req);
	} else {
		P_ERROR("Unable to disable controller " << this << ": " <<
