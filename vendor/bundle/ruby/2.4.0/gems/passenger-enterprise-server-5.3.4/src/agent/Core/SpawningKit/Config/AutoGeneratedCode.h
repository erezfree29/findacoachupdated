...
Passenger::SpawningKit::Config::internStrings() {
	...
	totalSize += concurrencyModel.size() + 1;
	...
	pos = appendData(pos, end, "\0", 1);
	pos = appendData(pos, end, concurrencyModel);
	pos = appendData(pos, end, "\0", 1);
	...
	tmpSize = concurrencyModel.size();
	concurrencyModel = StaticString(pos, tmpSize);
	pos += tmpSize + 1;
...
Passenger::SpawningKit::Config::validate(vector<StaticString> &errors) const {
	...
	if (OXT_UNLIKELY(concurrencyModel.empty())) {
		ok = false;
		errors.push_back(P_STATIC_STRING("concurrency_model may not be empty"));
	}
...
Passenger::SpawningKit::Config::getConfidentialFieldsToPassToApp() const {
	...
	doc["concurrency_model"] = concurrencyModel.toString();
	doc["thread_count"] = threadCount;
	doc["memory_limit"] = memoryLimit;
	doc["debugger_support"] = debuggerSupport;
...
Passenger::SpawningKit::Config::getNonConfidentialFieldsToPassToApp() const {
	...
	doc["concurrency_model"] = "<SECRET>";
	doc["thread_count"] = "<SECRET>";
	doc["memory_limit"] = "<SECRET>";
	doc["debugger_support"] = "<SECRET>";
