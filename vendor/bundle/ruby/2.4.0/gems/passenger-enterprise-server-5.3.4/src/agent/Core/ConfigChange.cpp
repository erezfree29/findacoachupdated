...
struct ConfigChangeRequest {
	...
	/***********/
	CloudUsageTracker::ConfigChangeRequest forCloudUsageTracker;
	/***********/
	...
	const PrepareConfigChangeCallback &callback)
{
	...
	if (passenger_enterprise_should_track_usage()) {
		ConfigKit::prepareConfigChangeForSubComponent(
			*workingObjects->tracker,
			coreSchema->cloudUsageTracker.translator,
			req->config->inspectEffectiveValues(),
			req->errors, req->forCloudUsageTracker);
	}
	/***************/
	...
	BOOST_NOEXCEPT_OR_NOTHROW
{
	...
	for (unsigned int i = 0; i < wo->threadWorkingObjects.size(); i++) {
		ThreadWorkingObjects *two = &wo->threadWorkingObjects[i];
		req->counter++;
		two->controller->setAgentConfig(coreConfig->inspectEffectiveValues());
	...
	/***************/
	if (passenger_enterprise_should_track_usage()) {
		wo->tracker->commitConfigChange(req->forCloudUsageTracker);
	}
	/***************/
