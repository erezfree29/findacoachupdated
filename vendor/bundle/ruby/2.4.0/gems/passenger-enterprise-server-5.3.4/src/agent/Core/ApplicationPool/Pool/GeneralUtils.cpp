		...
		if (requestTimedOut(waiter)) {
			postLockActions.push_back(
					boost::bind(GetCallback::call, waiter.callback, SessionPtr(),
							boost::make_shared<RequestQueueTimeoutException>(waiter.options.maxRequestQueueTimeMs)));
			continue;
		}
...
Pool::requestTimedOut(const GetWaiter &waiter) {
	if (OXT_LIKELY(waiter.options.maxRequestQueueTimeMs == 0)) {
		return false;
	} else {
		return (OXT_UNLIKELY(
				((SystemTime::getMonotonicUsecWithGranularity<SystemTime::GRAN_1SEC>() - waiter.startTimeUsec) / 1000)
						> waiter.options.maxRequestQueueTimeMs));
	}
}
