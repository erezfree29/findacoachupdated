...
licenseInvalidCallback(const string &reason) {
	...
	P_CRITICAL(reason); // for the user
...
initializeCloudUsageTracker() {
	...
	if (passenger_enterprise_should_track_usage()) {
		...
		P_INFO("Starting " PROGRAM_NAME " usage tracker using data directory " <<
			coreConfig->get("licensing_data_dir").asString());
		makeDirTree(coreConfig->get("licensing_data_dir").asString());

		wo->tracker = new CloudUsageTracker(coreSchema->cloudUsageTracker.schema,
			coreConfig->inspectEffectiveValues(),
			coreSchema->cloudUsageTracker.translator);
		wo->tracker->start();
	}
}
...
prestartWebApps() {
	...
	if (coreConfig->get("defer_port_binding").asBool()) {
		timeout = coreConfig->get("defer_port_binding_timeout").asString();
	}
	...
	boost::function<void ()> func = boost::bind(prestartWebApps,
		...
		prestartURLs,
		coreConfig->get("defer_port_binding").asBool(),
		coreConfig->get("defer_port_binding_params_file").asString(),
		timeout
	);
