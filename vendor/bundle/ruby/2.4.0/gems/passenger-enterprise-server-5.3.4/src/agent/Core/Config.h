 ...
 *   defer_port_binding                                              boolean            -          default(false),read_only
 *   defer_port_binding_params_file                                  string             -          read_only
 *   defer_port_binding_timeout                                      unsigned integer   -          default(90000),read_only
 ...
 *   licensing_check_certificate                                     boolean            -          default(true)
 *   licensing_data_dir                                              string             -          default
 *   licensing_proxy_url                                             string             -          -
 *   licensing_threshold                                             unsigned integer   -          default(288)
 *   licensing_url                                                   string             -          default("https://www.phusionpassenger.com/cloud_service/track_usage")
...
private:
	...
	static void validateEnterpriseConfig(const ConfigKit::Store &config, vector<ConfigKit::Error> &errors) {
		typedef ConfigKit::Error Error;
		string strval;
		unsigned int uintval;

		strval = config["default_concurrency_model"].asString();
		if (strval != "process" && strval != "thread") {
			errors.push_back(Error("'{{default_concurrency_model}}' must be one of: 'process', 'thread'"));
		}

		uintval = config["default_app_thread_count"].asUInt();
		if (uintval < 1) {
			errors.push_back(Error("'{{default_app_thread_count}}' must be at least 1"));
		}
	}
...
public:
	...
	Schema() {
		...
		// Add subschema: cloudUsageTracker
		cloudUsageTracker.translator.setPrefixAndFinalize("licensing_");
		addSubSchema(cloudUsageTracker.schema, cloudUsageTracker.translator);
		erase("licensing_auto_send");
		...
		/*******************/
		add("defer_port_binding", BOOL_TYPE, OPTIONAL | READ_ONLY, false);
		add("defer_port_binding_params_file", STRING_TYPE, OPTIONAL | READ_ONLY);
		add("defer_port_binding_timeout", UINT_TYPE, OPTIONAL | READ_ONLY, DEFAULT_START_TIMEOUT);

		addValidator(validateEnterpriseConfig);
		/*******************/
