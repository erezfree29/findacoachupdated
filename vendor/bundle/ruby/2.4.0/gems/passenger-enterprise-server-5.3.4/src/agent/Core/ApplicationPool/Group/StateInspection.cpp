...
Group::inspectConfigInAdminPanelFormat(Json::Value &result) const {
	...
	result["rolling_restarts"] = VAL(options.rollingRestart, false);
	result["resist_deployment_errors"] = VAL(options.ignoreSpawnErrors, false);
	result["concurrency_model"] = SVAL(options.concurrencyModel, "process");
	result["thread_count"] = VAL((Json::UInt) options.threadCount, 0);
	result["memory_limit"] = VAL((Json::UInt) options.memoryLimit, 0);
