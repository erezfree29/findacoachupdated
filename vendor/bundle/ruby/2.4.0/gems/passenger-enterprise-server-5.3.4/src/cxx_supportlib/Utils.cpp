...
static void
startProxy(const ResourceLocator &resourceLocator, const string &ruby,
	const string &deferPortBindingParamsFile)
{
	vector<string> command;
	command.push_back(resourceLocator.getHelperScriptsDir() + "/port-binding-proxy.rb");
	command.push_back(deferPortBindingParamsFile);
	runInternalRubyTool(resourceLocator, ruby, command);
}

void
prestartWebApps(const ResourceLocator &locator, const string &ruby,
				const vector<string> &prestartURLs, const bool deferPortBinding,
				const string &deferPortBindingParamsFile, const string &timeout)
{
	...
	if (deferPortBinding) {
		startProxy(locator, ruby, deferPortBindingParamsFile);
	}
}
